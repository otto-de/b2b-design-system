export const iconTypes = [
  '.DS_Store',
  'b2b_icon-360-degree',
  'b2b_icon-advice',
  'b2b_icon-allergen-sulfur-dioxide-sulfite',
  'b2b_icon-almond',
  'b2b_icon-archive-6m',
  'b2b_icon-archive',
  'b2b_icon-armchair',
  'b2b_icon-arrow-down',
  'b2b_icon-arrow-graph-axis',
  'b2b_icon-arrow-graph',
  'b2b_icon-arrow-left',
  'b2b_icon-arrow-long-down',
  'b2b_icon-arrow-long-left',
  'b2b_icon-arrow-long-right',
  'b2b_icon-arrow-long-up',
  'b2b_icon-arrow-misdirected',
  'b2b_icon-arrow-right',
  'b2b_icon-arrow-up',
  'b2b_icon-ascending-arrow-graph-axis',
  'b2b_icon-assembly',
  'b2b_icon-assortment',
  'b2b_icon-attachment',
  'b2b_icon-audio-description',
  'b2b_icon-augmented-reality',
  'b2b_icon-autonomous-team',
  'b2b_icon-bank-details',
  'b2b_icon-bar-graph-ascending-magnifying-glass',
  'b2b_icon-bar-graph-ascending',
  'b2b_icon-bar-graph-axis',
  'b2b_icon-barley',
  'b2b_icon-basket-sale',
  'b2b_icon-basket',
  'b2b_icon-basketball',
  'b2b_icon-bikini',
  'b2b_icon-blogs',
  'b2b_icon-box-scale',
  'b2b_icon-brandshop',
  'b2b_icon-brazil-nut',
  'b2b_icon-browser-click',
  'b2b_icon-browser-content-skyscraper',
  'b2b_icon-browser-content',
  'b2b_icon-browser-duration',
  'b2b_icon-browser-hand-pointing',
  'b2b_icon-browser-list-click',
  'b2b_icon-browser-magnifying-glass-info',
  'b2b_icon-browser-person-click',
  'b2b_icon-browser-psm',
  'b2b_icon-browser-target',
  'b2b_icon-browser-teaser',
  'b2b_icon-bullet-list',
  'b2b_icon-burger',
  'b2b_icon-bus',
  'b2b_icon-callback',
  'b2b_icon-camera',
  'b2b_icon-cashback',
  'b2b_icon-cashew-nut',
  'b2b_icon-catalogue',
  'b2b_icon-categories',
  'b2b_icon-celery',
  'b2b_icon-certificate',
  'b2b_icon-chair',
  'b2b_icon-chat-ellipsis',
  'b2b_icon-chat',
  'b2b_icon-chatbot',
  'b2b_icon-check-bold',
  'b2b_icon-check',
  'b2b_icon-chicken',
  'b2b_icon-city',
  'b2b_icon-clear',
  'b2b_icon-clipboard-check',
  'b2b_icon-close-bold',
  'b2b_icon-close',
  'b2b_icon-closed-captions',
  'b2b_icon-clover-four-leaf',
  'b2b_icon-code-management',
  'b2b_icon-cogwheel-circular-arrow',
  'b2b_icon-cogwheel-pen',
  'b2b_icon-contact-mail',
  'b2b_icon-contrast',
  'b2b_icon-cookie',
  'b2b_icon-copy',
  'b2b_icon-cosmetics',
  'b2b_icon-cow',
  'b2b_icon-crab',
  'b2b_icon-critique',
  'b2b_icon-crown-circle',
  'b2b_icon-curtain',
  'b2b_icon-cut-out',
  'b2b_icon-dashboard',
  'b2b_icon-data-filter',
  'b2b_icon-decision',
  'b2b_icon-deer',
  'b2b_icon-delete',
  'b2b_icon-delivery-24h',
  'b2b_icon-delivery-curb',
  'b2b_icon-delivery-desired-date-selection',
  'b2b_icon-delivery-desired-date',
  'b2b_icon-delivery-express',
  'b2b_icon-delivery-flat',
  'b2b_icon-delivery-on-hangers',
  'b2b_icon-delivery-preferred-place',
  'b2b_icon-design-assets',
  'b2b_icon-desktop-code',
  'b2b_icon-desktop-phone-ads-click',
  'b2b_icon-desktop-phone-ads',
  'b2b_icon-desktop-phone',
  'b2b_icon-desktop',
  'b2b_icon-devices-monitor-laptop-tablet',
  'b2b_icon-diamond-circle',
  'b2b_icon-dish-feature-lactose-free',
  'b2b_icon-dislike',
  'b2b_icon-disposal',
  'b2b_icon-document-magnifying-glass',
  'b2b_icon-document-sign',
  'b2b_icon-document-speaker',
  'b2b_icon-downloads',
  'b2b_icon-dress',
  'b2b_icon-dungarees',
  'b2b_icon-duplicate',
  'b2b_icon-duration',
  'b2b_icon-e-invoice',
  'b2b_icon-edit-pad',
  'b2b_icon-edit',
  'b2b_icon-egg',
  'b2b_icon-ellipsis-vertical',
  'b2b_icon-ellipsis',
  'b2b_icon-email-verification-confirmed-safe',
  'b2b_icon-email-verification-confirmed',
  'b2b_icon-email-verification-sent-safe',
  'b2b_icon-email-verification-sent',
  'b2b_icon-empty-hint',
  'b2b_icon-end-to-end',
  'b2b_icon-error-article-image',
  'b2b_icon-error-hint',
  'b2b_icon-error',
  'b2b_icon-euro-sale',
  'b2b_icon-euro',
  'b2b_icon-event',
  'b2b_icon-events',
  'b2b_icon-expand',
  'b2b_icon-factory',
  'b2b_icon-faq',
  'b2b_icon-feedback',
  'b2b_icon-filter',
  'b2b_icon-fish',
  'b2b_icon-fitness',
  'b2b_icon-flag-chequered',
  'b2b_icon-flame',
  'b2b_icon-flexible-work',
  'b2b_icon-flip',
  'b2b_icon-flyer',
  'b2b_icon-food-processor',
  'b2b_icon-fullscreen',
  'b2b_icon-gallery-view',
  'b2b_icon-gamepad',
  'b2b_icon-garlic',
  'b2b_icon-gavel',
  'b2b_icon-gender-intersex',
  'b2b_icon-globe-hand',
  'b2b_icon-globe',
  'b2b_icon-gluten',
  'b2b_icon-glutenfree',
  'b2b_icon-hand-high-five',
  'b2b_icon-hand-pointing-up',
  'b2b_icon-hand-raised',
  'b2b_icon-handshake',
  'b2b_icon-hashtag',
  'b2b_icon-hazelnut',
  'b2b_icon-hearing-disability',
  'b2b_icon-hide',
  'b2b_icon-high-heels',
  'b2b_icon-home',
  'b2b_icon-iceberg',
  'b2b_icon-info-small',
  'b2b_icon-info',
  'b2b_icon-inplace-zoom-reverse',
  'b2b_icon-inplace-zoom',
  'b2b_icon-insurance',
  'b2b_icon-kamut',
  'b2b_icon-keyboard',
  'b2b_icon-laptop-os',
  'b2b_icon-laptop-phone-ads',
  'b2b_icon-laptop-phone-youtube',
  'b2b_icon-laptop-phone',
  'b2b_icon-laptop',
  'b2b_icon-leaving-page-50',
  'b2b_icon-leaving-page',
  'b2b_icon-letter-opened',
  'b2b_icon-light-bulb',
  'b2b_icon-like-fingers',
  'b2b_icon-like-product',
  'b2b_icon-like',
  'b2b_icon-line-graph',
  'b2b_icon-link',
  'b2b_icon-list-view',
  'b2b_icon-location',
  'b2b_icon-lock-closed-check',
  'b2b_icon-lock-open',
  'b2b_icon-lupine',
  'b2b_icon-macadamia',
  'b2b_icon-material-samples',
  'b2b_icon-measurement',
  'b2b_icon-megaphone',
  'b2b_icon-mental-disability',
  'b2b_icon-menu',
  'b2b_icon-mic',
  'b2b_icon-milk',
  'b2b_icon-minus',
  'b2b_icon-module-operations',
  'b2b_icon-money-banknotes',
  'b2b_icon-money-coins-arrow',
  'b2b_icon-money-coins',
  'b2b_icon-money-document',
  'b2b_icon-money-plus',
  'b2b_icon-money-stack',
  'b2b_icon-money',
  'b2b_icon-moon',
  'b2b_icon-move',
  'b2b_icon-movie',
  'b2b_icon-mustard',
  'b2b_icon-nav',
  'b2b_icon-new',
  'b2b_icon-newsletter-sign-up',
  'b2b_icon-notification-strikethrough',
  'b2b_icon-notification',
  'b2b_icon-nuts',
  'b2b_icon-oat',
  'b2b_icon-ogh-change',
  'b2b_icon-order-cancelled',
  'b2b_icon-order-delivered',
  'b2b_icon-order-form',
  'b2b_icon-order-management',
  'b2b_icon-order-misdirected',
  'b2b_icon-order-on-road',
  'b2b_icon-order-packed',
  'b2b_icon-order-payment',
  'b2b_icon-order-received',
  'b2b_icon-order-return-arrow',
  'b2b_icon-order-return',
  'b2b_icon-order-shipped',
  'b2b_icon-order-state-unknown',
  'b2b_icon-order-stored',
  'b2b_icon-order',
  'b2b_icon-ordered-list',
  'b2b_icon-otto-app',
  'b2b_icon-otto-campus',
  'b2b_icon-otto-change',
  'b2b_icon-otto-company',
  'b2b_icon-otto-laptop',
  'b2b_icon-otto-logo',
  'b2b_icon-otto-up-points',
  'b2b_icon-otto-up',
  'b2b_icon-out-of-home-digital',
  'b2b_icon-out-of-home',
  'b2b_icon-paper-plane',
  'b2b_icon-pause',
  'b2b_icon-payment-credit-card',
  'b2b_icon-payment-invoice',
  'b2b_icon-payment-pause',
  'b2b_icon-payment-plan-protection',
  'b2b_icon-payment-plan',
  'b2b_icon-pdf',
  'b2b_icon-peanut',
  'b2b_icon-pecan-nut',
  'b2b_icon-pending',
  'b2b_icon-person-access',
  'b2b_icon-person-arrow-graph',
  'b2b_icon-person-callcenter',
  'b2b_icon-person-check',
  'b2b_icon-person-colleagues',
  'b2b_icon-person-community',
  'b2b_icon-person-development',
  'b2b_icon-person-exchange',
  'b2b_icon-person-female',
  'b2b_icon-person-influencer',
  'b2b_icon-person-info',
  'b2b_icon-person-magnifying-glass',
  'b2b_icon-person-male',
  'b2b_icon-person-rating',
  'b2b_icon-person-support',
  'b2b_icon-person-switch',
  'b2b_icon-person-wheelchair',
  'b2b_icon-person-workshop',
  'b2b_icon-person',
  'b2b_icon-phone-permission',
  'b2b_icon-phone-play',
  'b2b_icon-phone-shopping-cart',
  'b2b_icon-phone',
  'b2b_icon-pig',
  'b2b_icon-pin',
  'b2b_icon-pinch-gesture',
  'b2b_icon-pinch',
  'b2b_icon-pistachio',
  'b2b_icon-plant',
  'b2b_icon-play-1',
  'b2b_icon-play',
  'b2b_icon-plus-hint',
  'b2b_icon-plus',
  'b2b_icon-price-alert',
  'b2b_icon-price-tag-premium',
  'b2b_icon-price-tag',
  'b2b_icon-print',
  'b2b_icon-puzzle',
  'b2b_icon-qr-code',
  'b2b_icon-question-hint',
  'b2b_icon-question',
  'b2b_icon-radio',
  'b2b_icon-rating-empty',
  'b2b_icon-rating-filled',
  'b2b_icon-rating-half',
  'b2b_icon-rating-order',
  'b2b_icon-rating-payment',
  'b2b_icon-reach',
  'b2b_icon-refresh',
  'b2b_icon-remote-work',
  'b2b_icon-repair',
  'b2b_icon-reply',
  'b2b_icon-retirement-provision',
  'b2b_icon-rocket',
  'b2b_icon-room',
  'b2b_icon-rye',
  'b2b_icon-sabbatical',
  'b2b_icon-sale',
  'b2b_icon-save',
  'b2b_icon-screenshot',
  'b2b_icon-sea',
  'b2b_icon-search-enter',
  'b2b_icon-search-navigation',
  'b2b_icon-search',
  'b2b_icon-seo',
  'b2b_icon-service-info',
  'b2b_icon-sesame',
  'b2b_icon-settings',
  'b2b_icon-share-ios',
  'b2b_icon-share',
  'b2b_icon-sheep',
  'b2b_icon-shell',
  'b2b_icon-shirt',
  'b2b_icon-shopping-cart-euro',
  'b2b_icon-shopping-cart-plus',
  'b2b_icon-shopping-cart',
  'b2b_icon-show',
  'b2b_icon-skip-link',
  'b2b_icon-smiley-angry',
  'b2b_icon-smiley-confused',
  'b2b_icon-smiley-dizzy',
  'b2b_icon-smiley-happy',
  'b2b_icon-smiley-negative',
  'b2b_icon-smiley-neutral',
  'b2b_icon-smiley-pensive',
  'b2b_icon-smiley-positive',
  'b2b_icon-smiley-relieved',
  'b2b_icon-smiley-sad',
  'b2b_icon-smiley-smiling-with-smiling-eyes',
  'b2b_icon-smiley-smiling',
  'b2b_icon-smiley-squinting',
  'b2b_icon-smiley-winking',
  'b2b_icon-smiley-worried',
  'b2b_icon-sms-info',
  'b2b_icon-sneaker',
  'b2b_icon-sort-ascending',
  'b2b_icon-sort-descending',
  'b2b_icon-sort',
  'b2b_icon-soy',
  'b2b_icon-sparkles',
  'b2b_icon-speech-bubble-eye',
  'b2b_icon-speech-bubble-sparkles',
  'b2b_icon-speech-disability',
  'b2b_icon-spelt',
  'b2b_icon-sphere',
  'b2b_icon-star-circle',
  'b2b_icon-stationary',
  'b2b_icon-studies',
  'b2b_icon-success-hint-filled',
  'b2b_icon-success',
  'b2b_icon-sulfur-dioxide-sulfite',
  'b2b_icon-sun',
  'b2b_icon-sustainability',
  'b2b_icon-sustainable-animal-welfare',
  'b2b_icon-sustainable-delivery',
  'b2b_icon-sustainable-energy-efficient',
  'b2b_icon-sustainable-natural-cosmetics',
  'b2b_icon-sustainable-organic-food',
  'b2b_icon-sustainable-organic-materials',
  'b2b_icon-sustainable-production',
  'b2b_icon-sustainable-recyclable-design',
  'b2b_icon-sustainable-recycling',
  'b2b_icon-sustainable-resource',
  'b2b_icon-sustainable-social-initiatives',
  'b2b_icon-swipe-gesture',
  'b2b_icon-sync-device',
  'b2b_icon-t-shirt',
  'b2b_icon-tablet',
  'b2b_icon-tableware',
  'b2b_icon-target-group',
  'b2b_icon-teddy',
  'b2b_icon-test-result',
  'b2b_icon-text-scale',
  'b2b_icon-theming',
  'b2b_icon-touch',
  'b2b_icon-town',
  'b2b_icon-trash',
  'b2b_icon-trend-down',
  'b2b_icon-trend-left',
  'b2b_icon-trend-right',
  'b2b_icon-trend-up',
  'b2b_icon-trousers',
  'b2b_icon-trusted',
  'b2b_icon-tv-smart',
  'b2b_icon-tv-target',
  'b2b_icon-tv',
  'b2b_icon-universal-access',
  'b2b_icon-unpin',
  'b2b_icon-upload',
  'b2b_icon-user-management',
  'b2b_icon-vacation',
  'b2b_icon-variant-content',
  'b2b_icon-vegan',
  'b2b_icon-vegetarian',
  'b2b_icon-viewtoggle-tiles',
  'b2b_icon-village',
  'b2b_icon-visual-disability',
  'b2b_icon-voice-sparkles',
  'b2b_icon-volume-off',
  'b2b_icon-volume-on',
  'b2b_icon-voucher',
  'b2b_icon-walnut',
  'b2b_icon-warning-hint',
  'b2b_icon-warning',
  'b2b_icon-warranty',
  'b2b_icon-washing-machine',
  'b2b_icon-webhook',
  'b2b_icon-whatsapp',
  'b2b_icon-wheat',
  'b2b_icon-wifi',
  'b2b_icon-wishlist-active',
  'b2b_icon-wishlist',
  'b2b_icon-work-life',
  'b2b_icon-workshop',
  'b2b_icon-youtube',
  'b2b_icon-zoom-in',
  'b2b_icon-zoom-out',
  'b2b_logo-carrier-dhl',
  'b2b_logo-carrier-gls',
  'b2b_logo-carrier-hermes',
  'b2b_logo-carrier-ups',
] as const;
export type IconName = (typeof iconTypes)[number];
