...
Options can be organized into categories for better readability and user experience. Use the `b2b-custom-dropdown-option-category` component to group related options together. Each category has a label that identifies the group. This is useful when you have a large number of options that can be logically grouped.

<Canvas of={CustomDropdownStories.WithOptionCategory} />
