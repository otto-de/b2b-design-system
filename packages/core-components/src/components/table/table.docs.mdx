...
This event is emitted whenever the values in a rowgroup change (e.g. in an accordion).
It emits the associated group and an array of values currently selected in that group.

☝️ You should only subscribe to it if you have nested data in an accordion.
