...
<Canvas of={TooltipStories.DefaultHoverTooltip} />
...
<Canvas of={TooltipStories.FocusTooltip} />

Additionally, the tooltip will try to fit it's content up to a maximum width
of `17.5rem` or `280px`, then it will overflow to the next line.
...
```html
<b2b-tooltip><b2b-icon-100 slot="content"></b2b-icon-100></b2b-tooltip>
```
