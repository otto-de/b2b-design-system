...
The Input Component provides several slots to enhance its functionality and appearance.
You can use these slots to add prefixed or suffixed text, icons, or even customize the label.

If you want to add text in the `Start` or `End` slots, please use a `<span>` tag as this
will automatically add a border to separate it from the user input. Note that it is not necessary to use B2B Icons,
the slots work with any icon.
...
<Canvas of={InputStories.Suffix} sourceState="shown" />

### Label

If you need the label to contain something other than plain text you may use the label slot like so:

<Canvas of={InputStories.Label} sourceState="shown" />

### Hint

If you need the hint to contain something other than plain text you may use the hint slot. Only the following elements are supported in this slot:
...
Any other elements will not be displayed.

<Canvas of={InputStories.HintSlot} sourceState="shown" />
...
<Canvas of={InputStories.CenterAligned} sourceState="shown" />

#### Right aligned text

<Canvas of={InputStories.RightAligned} sourceState="shown" />
